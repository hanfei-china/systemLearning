...
import {ROW_COL_BTN, SIDE_STOP_BTN} from '../../config/defaultImages';

/**
 * 侧停按钮
 */
export default class SideStopButton extends Module{

    constructor(config){
        super('sideStopButton');

        let title, funcType, appId, contentClass, counselClass, memberQueryBuilders, privilege,
            content, onlineForm, icon, pagePath, informationExchangeType;
        title = new Attribute('textAttr','侧停按钮')
            .addAttributes({
                title: '按钮名称',
                placeholder: '请输入按钮名称',
                maxLength: 10,
        ...
        icon = new Attribute('imageAttr',SIDE_STOP_BTN)
            .addAttributes({
                title: '按钮图标',
                width: 40,
                height: 40,
         ...
         funcType = new Attribute('funcTypeAttr', 6)
            .addAttributes({
                title: '按钮功能',
        ...
        let attributes = {title, funcType, appId, contentClass, counselClass,
            content, memberQueryBuilders, onlineForm, informationExchangeType, pagePath, privilege, icon};
