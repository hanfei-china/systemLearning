<template>
  <div class="contactPerson">
    ...
    <div class="contactPerson-left">
      ...
      <div class="contactPerson-left__top">
        ...
        <div v-if="allMemberList.length>0" @click="ifSelectAll=!ifSelectAll" class="contactPerson-left__top__right">
          <img src="./image/ifSelectAll.png" v-if="!ifSelectAll" />
          <img src="./image/selectAll.png" v-else>
          <span>选择所有联系人 共{{allMemberList.length}}人</span>
        </div>
      </div>
      <!-- 搜索框 -->
      <div class="contactPerson-left__search">
        <el-input placeholder="姓名、手机" prefix-icon="el-icon-search" v-model="searchInfo"></el-input>
      </div>
      <!-- 联系人展示模块 -->
      ...
      <!-- <div class="contactPerson-left_tag">
        <div v-if="tagsList.length>0">标签</div>
        <ul>
          <li
            ...
            :class="{'selectTag':selectTagsList.indexOf(item.id)>=0}"
            @click="clickTag(item.id)"
          >{{item.title}}&nbsp;(&nbsp;{{item.memberNum}}&nbsp;)</li>
        </ul>
      </div> -->
    </div>
    <!-- 中间的边界 -->
    ...
    <div class="contactPerson-right">
      <!-- 头部 -->
      <div class="contactPerson-right__top">
...
export default {
  ...
  created() {
    ...
    this.allMemberList=JSON.parse(JSON.stringify(this.sendSmsList));
    this.ifSelectAll=true; 
  },
  methods: {
      //勾选所有联系人
    ...
    getPersonList(val) {
      if (val) {
        this.personList = [];
        this.$request
          .get("/api/admin/activityInfo/getActivityInfoByCondition?content=" + val+'&activityId='+this.activityId)
          .then(res => {
            this.personList = JSON.parse(JSON.stringify(res.data));
            this.personList.forEach(item=>{
              //数组格式处理
              item.telphone=item.phone;
