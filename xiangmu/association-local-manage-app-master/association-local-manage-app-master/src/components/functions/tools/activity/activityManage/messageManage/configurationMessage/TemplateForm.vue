<template>
    <el-container class="absolute-fill-parent">
        ...
        <el-main class="flex">
                ...
                <vue-scroll>
                    <el-form size="small" label-width="100px">
                        ...
                        <el-form-item label="邮件内容：" required>
                            <div>
                                <div style="line-height: 32px;background-color: #e4e4e4;padding: 5px;border: 1px solid #DCDFE6;border-bottom: none;">
                                    <span>添加字段：</span>
                                    <el-select style="width: 160px;" v-model="fieldSelect" @change="selectFields">
                                        <el-option v-for="item in fieldList" :key="item.id" :value="item.id"
                                                   :label="item.name"/>
                                    </el-select>
                                </div>
                                <div>
            ...
            <div class="border flex flex-column" style="width: 500px;margin-left: 20px;">
                <div class="flex" style="line-height: 32px;padding: 10px;">
                    <span>主题</span>
                    <div class="flex-fill-in-the-remaining-space" style="padding: 0 0 0 10px;">
                        <div class="border" style="width: 100%;height: 100%;box-sizing: border-box;">{{subject}}</div>
                    </div>
                </div>
                <div class="flex-fill-in-the-remaining-space flex" style="line-height: 32px;padding: 10px;">
                    <span>正文</span>
                    <div class="flex-fill-in-the-remaining-space" style="padding: 0 0 0 10px;">
                ...
                <div class="flex" style="line-height: 32px;padding: 10px;">
                    <span>附件</span>
                    <div class="flex-fill-in-the-remaining-space" style="padding: 0 0 0 10px;">
                        <p v-for="item in attachPaths" :key="item.uid">{{item.name}}</p>
                    </div>
                </div>
            </div>
        </el-main>
        ...
        methods: {
            getData(){
                if(this.templateId){
                    this.$request.get('/admin/communication/template/getTemplate',{params: {
                        ...
                        this.attachPaths = JSON.parse(data.template.attachPaths);
            ...
            selectFields(id){
                ...
                for(let item of this.fieldList){
                    if(item.id === id){
                        ...
                        this.editor.execCommand('insertHtml',`{{${item.name}}}`);
                    }
                }
            },
            beforeUpload(file){
            ...
            saveData() {
                ...
                if(this.templateId){
                    ...
                    this.$request.put('/admin/communication/template/updateTemplate',data).then(()=>{
                        this.$message.success('模版修改成功');
                        this.$emit('refresh');
                        this.$emit('close');
                    });
                }else{
                    this.$request.post('/admin/communication/template/addTemplate',data).then(()=>{
