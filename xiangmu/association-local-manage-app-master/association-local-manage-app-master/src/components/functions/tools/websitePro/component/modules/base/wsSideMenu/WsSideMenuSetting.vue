<template>
    <setting-container ref="setting" :width.sync="module.attributes.layoutGroup.attributes.layout.value.width"
                       ...
                       :title="module.title" :inner="false" :can-delete="canDelete"
                       @setting="settingModule(module)" @delete="$emit('delete')"
                       @setting-if-setting="settingIfSetting(module)" @refLineParams="getRefLineParams">
        <ws-side-menu class="lrd-dom-drag" :module="module" design/>
    </setting-container>
</template>

<script>
    import SettingContainer from "../../SettingContainer";
    import WsSideMenu from "./WsSideMenu";
    export default {
        name: "WsSideMenuSetting",
        components: {WsSideMenu, SettingContainer},
        props:{
            module: Object,
            canDelete: {
                type: Boolean,
