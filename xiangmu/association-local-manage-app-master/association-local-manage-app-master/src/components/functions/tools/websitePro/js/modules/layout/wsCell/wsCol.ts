...
export default class WsCol extends Module {

    constructor(config: { id: any, title: any, [id: string]: any }) {
        let id = config.id;
        let title = config.title || '列';
        super('wsCol', id, title, '1.0');
        this.initAttribute();
    ...
    initAttribute() {
        ...
        let width = new Attribute('列宽度', 'number', 1).addAttributes({
            titleStyle:{
                paddingRight: '10px'
            }
        });
        ...
        layoutGroup.setAttributes({width, margin});
