...
export default class WsSideMenuItem extends Module{

    constructor(config: {id:any,title:any,[id:string]: any}) {
        let id = config.id;
        let title = config.title || '子项';
        super('wsSideMenuItem',id,title,'1.0');
        this.initAttribute();
    ...
    initAttribute(){
        ...
        let routeId = new Attribute('路由id','text','').addAttributes({
        ...
        otherGroup.setAttributes({customClasses,routeId});
