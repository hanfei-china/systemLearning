<template>
    <div v-show="show" class="website-pro-flex">
        ...
        <div style="height: 28px;" class="flex-fill-in-the-remaining-space">
            <el-select style="width: 100%;" placeholder="请选择查询" :clearable="clearable" ref="select" size="mini" v-model="attribute.value">
                <el-option v-for="item in list" :key="item.id" :value="item.id" :label="item.queryTitle"/>
            </el-select>
        </div>
    </div>
</template>
