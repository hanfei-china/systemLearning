    ...
    <el-container style="padding: 0 16px 16px;" class="absolute-fill-parent background-color-white">
        ...
        <el-main class="unset-padding position-relative">
            <el-draggable-table :list="list" :el-table-props="elTableProps" class="custom-el-table header-gray"
                ...
                <el-table-column class-name="item-un-sort" label="序号" type="index" align="center" width="50"/>
                <el-table-column class-name="item-un-sort" prop="title" show-tooltip-when-overflow label="分类名称"/>
                <el-table-column class-name="item-un-sort" label="分类备注" show-tooltip-when-overflow>
                    <template v-slot="{row}">
                        <span v-hyperlink="{innerHtml:row.description}" />
                    </template>
                </el-table-column>
                <el-table-column class-name="item-un-sort" label="创建时间" show-tooltip-when-overflow class="un-sort-item" width="150px">
                    <template v-slot="{row}">
        ...
        <el-dialog :visible.sync="classFormVisible" :title="classFormTitle"
            ...
            <online-donation-class-form v-if="classFormVisible" :class-info="classFormInfo" :seq="list.length"
...
<script>
    import OnlineDonationClassForm from "./OnlineDonationClassForm";
    import ElDraggableTable from "../../../../util/elTableDraggable/ElDraggableTable";
    export default {
        name: "OnlineDonationClassManage",
        components: {ElDraggableTable, OnlineDonationClassForm},
        data(){
            return {
                list:[],
                title:'',
        ...
        methods:{
            sort(){
                ...
                this.$request.put('/api/admin/onlineDonationClass/sort',{ids}).then(()=>{
                    this.$message.success("分类排序成功");
                })
            },
            getList(){
                this.$request.get('/api/admin/onlineDonationClass/list',{
            ...
            deleteClass(row){
                this.$msgbox.confirm('删除不可恢复，是否继续删除？','提示',{type: 'warning',confirmButtonText:'确认删除'}).then(()=>{
                    this.$request.delete('/api/admin/onlineDonationClass',{
                        params:{id:row.id}
                    }).then((data)=>{
                        if (data.success) {
                            this.getList();
