<template>
    <el-container style="padding: 0 16px 16px;" class="absolute-fill-parent background-color-white">
        ...
        <el-dialog :visible.sync="typeVisible" :title="visibleTitle" append-to-body :close-on-click-modal="false" width="800px">
            <dues-type-form v-if="typeVisible" :id="typeId" :copySeq="seq" @close="close" @refresh="refresh"></dues-type-form>
        </el-dialog>
    </el-container>
</template>

<script>
    import DuesTypeForm from "./DuesTypeForm";
    import Sortable from 'sortablejs';
    export default {
        name: "DuesTypeManage",
        components: {DuesTypeForm},
        data() {
            return {
                list: [],
                name: '',
