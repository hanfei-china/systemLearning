<template>
  <el-container class="unset-padding">
    <el-header>
      <el-input style="width: 230px" v-model="searchInfo" placeholder="请输入姓名或邮箱进行搜索" clearable></el-input>
    </el-header>
    <el-main>
      <el-table
        :data="memberList"
...
export default {
  ...
  computed: {
    copySendMemParm() {
      ...
      params.sendStatus  = this.sendMemParm.sendStatus
  ...
  methods: {
    ...
    getSendList() {
      this.$request.get('/api/admin/emailTelPhone/getSendEmailDetailPage', {params: this.copySendMemParm})
        .then(res => {
