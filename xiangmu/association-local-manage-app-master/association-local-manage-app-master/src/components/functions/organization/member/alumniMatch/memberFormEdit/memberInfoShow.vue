<template>
  <el-container class="absolute-fill-parent border-top cusdia" v-loading="loading">
    ...
    <el-main class="unset-padding position-relative" v-if="memberInfo">
      <vue-scroll @handle-scroll="click">
        <div style="padding:20px;">
          ...
          <div class="fieldbox">
            <h5>基本信息
              <!-- <div v-if="OPprivileges.includes('201010003')"> -->
              <div>
            ...
            <div class="rows">
              <div class="rowpart" v-for="(item,index) in basicFieldList" :key="index">
                ...
                <div v-if="addStatus=='basic'">
                  <el-date-picker size="mini" v-if="item.topicType==5" v-model="model[item.code]" value-format="yyyy-MM-dd" type="date" placeholder="选择日期"></el-date-picker>
                  <city-choose size="mini" v-else-if="item.topicType==6" v-model="model[item.code]" :show-city="item.showCity"/>
          ...
          <div class="fieldbox">
            <!-- <h5>在校经历<el-button v-if="OPprivileges.includes('201010003')" style="float:right;font-style:normal;" type="text" @click="addExp">添加经历</el-button></h5> -->
            <h5>在校经历<el-button style="float:right;font-style:normal;" type="text" @click="addExp">添加经历</el-button></h5>
            <div class="newrows" v-if="addStatus=='experience'">
              <!-- :disabled="experienceType&&experienceType=='教职工'&&(!!teacherExperienceObj.id)" -->
              <el-select  v-model="experienceType" @change="changeExp" placeholder="请选择" size="mini">
              ...
              <div class="flexfield" v-if="experienceType&&experienceType=='教职工'">
                <div class="fielditem" v-for="(itemfield,itemindex) in teacherExperienceField" :key="itemindex">
                  ...
                  <member-edit-field-new :experienceType="experienceType" :studentExperienceObj="studentExperienceObj" :memberFields="teacherExperienceField" :code="itemfield.code" :config="itemfield" v-model="teacherExperienceObj[itemfield.code]" :isComposeField="true" :itemindex="itemindex" @changeMemberFieldsDetail="changeMemberFieldsDetail" />
                </div>
              </div>
              <div class="flexfield" v-else-if="experienceType&&experienceType!=='教职工'">
                <div class="fielditem" v-for="(itemfield,itemindex) in studentExperienceField" :key="itemindex">
                  <span class="custom-el-form-item-label">{{itemfield.name}}：</span>
                  <member-edit-field-new :experienceType="experienceType" :studentExperienceObj="studentExperienceObj" :memberFields="studentExperienceField" :code="itemfield.code" :config="itemfield" v-model="studentExperienceObj[itemfield.code]" :isComposeField="true" :itemindex="itemindex" @changeMemberFieldsDetail="changeMemberFieldsDetail" />
                </div>
              </div>
            </div>
            <div class="expbox" v-else>
              <div class="rows" v-for="(item,index) in model.studentExperience" :key="index" v-if="studentExperienceFieldCode.length>0">
                <div class="row1">
                  <span><i></i></span><p v-if="studentExperienceFieldCode.includes('college')">{{item.college}}</p><strong><template v-if="studentExperienceFieldCode.includes('entranceDate')">{{item.entranceDate|filterYear}}</template>-<template v-if="studentExperienceFieldCode.includes('graduationDate')">{{item.graduationDate|filterYear}}</template></strong>
                  <!-- <div class="operatebox" v-if="OPprivileges.includes('201010003')"> -->
                ...
                <div class="row2">
                  <div v-if="studentExperienceFieldCode.includes('education')">{{item.education}}</div><div v-if="item.major&&studentExperienceFieldCode.includes('major')">{{item.major}}</div><div v-if="item.cless&&studentExperienceFieldCode.includes('cless')">{{item.cless}}</div><div v-if="item.studentNo&&studentExperienceFieldCode.includes('studentNo')">{{item.studentNo}}</div>
                </div>
              </div>
              <div class="rows" v-for="(item,index) in model.teacherExperience" :key="item.id" v-if="teacherExperienceFieldCode.length>0">
                <div class="row1">
                  <span><i></i></span><p v-if="teacherExperienceFieldCode.includes('department')">{{item.department}}</p><strong>任职</strong>
                  <!-- <div class="operatebox" v-if="OPprivileges.includes('201010003')"> -->
                  <div class="operatebox">
                ...
                <div class="row2">
                  <div v-if="item.job&&teacherExperienceFieldCode.includes('job')">{{item.job}}</div><div v-if="item.jobNo&&teacherExperienceFieldCode.includes('jobNo')">{{item.jobNo}}</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="fieldbox" v-if="workingFieldCode.length>0">
            ...
            <div v-if="addStatus=='working'" class="rows">
              <div class="rowpart" v-for="(itemfield,itemindex) in workingField" :key="itemindex">
                <span class="custom-el-form-item-label">{{itemfield.name}}：</span>
                <el-cascader :show-all-levels="false" v-if="itemfield.topicType==20" :placeholder="'下拉选择'+itemfield.name" size="mini" filterable clearable v-model="model[itemfield.code]" :options="industryList" :props="props"/>
            ...
            <div v-else class="rows">
              <div class="row1">
                <span><i></i></span><p v-if="model.workingCompany&&workingFieldCode.includes('workingCompany')">{{model.workingCompany}}</p><strong  v-if="model.unitCharacter&&workingFieldCode.includes('unitCharacter')" style="font-weight: normal;font-size: 12px;color: #C6C6C6;">{{model.unitCharacter}}</strong>
              </div>
              <div class="row2" :class="model.workingCompany?'':'noCompany'">
                <div v-if="model.industry&&workingFieldCode.includes('industry')">{{model.industry}}</div><div v-if="model.post&&workingFieldCode.includes('post')">{{model.post}}</div>
              </div>
            </div>
          </div>
          ...
          <div class="fieldbox">
            ...
            <div class="rows">
              <div class="rowpart"><span>账户状态：</span>
                <div style="color:#3FA1B6;" v-if="model.status=='1'">账户已激活</div><div style="color:#ECA038;" v-else>账户未激活</div>
              </div>
              <div class="rowpart"><span>最近登录时间：</span><div v-if="userInfo&&userInfo.lastLoginTime">{{userInfo.lastLoginTime|filterTime}}</div></div>
              <div class="rowpart"><span>注册方式：</span><div>{{model.source}}</div></div>
              <div class="rowpart"><span>注册时间：</span><div v-if="userInfo&&userInfo.createTime">{{userInfo.createTime|filterTime}}</div></div>
            </div>
          </div>
        </div>
      </vue-scroll>
...
export default {
        name: "memberInfoShow",
        components: {MemberGroupMultiTree, AllMemberFieldChoose, MemberEditFieldNew,CityChoose},
        props: {
            userId: {
                type: String,
                default: ''
        ...
        data() {
            return {
                ...
                studentExperienceField:[
                    ...
                    {code:"graduationDate",name:"毕业年",topicType:5,needClear:true},
        ...
        methods: {
            ...
            changeExp(){
                for(var i=0;i<this.studentExperienceField.length;i++){
                    if(this.studentExperienceField[i].needClear){
                        this.$set(this.studentExperienceObj,this.studentExperienceField[i].code,"")
                    }
                }
            },
            deleteStudentExp(item,index){
                this.$msgbox.confirm('删除操作是不可逆的，是否继续删除？','提示',{type: 'warning',confirmButtonText:'确认删除'}).then(()=>{
                    this.$request.delete('/api/admin/member/v2/deleteMemberStudentExperience',{ params:{id:item.id} }).then(()=>{
            ...
            deleteTeacherExp(item,index){
                this.$msgbox.confirm('删除操作是不可逆的，是否继续删除？','提示',{type: 'warning',confirmButtonText:'确认删除'}).then(()=>{
                    this.$request.delete('/api/admin/member/v2/deleteMemberTeacherExperience',{ params:{id:item.id} }).then(()=>{
                        this.getMemberFields()
                        this.$message.success('删除成功');
                    })
                }).catch(()=>{});
            ...
            saveBasic(){
              var fieldValues ={}
              this.basicFieldList.map(item=>{ fieldValues[item.code] = this.model[item.code] })
              fieldValues.id = this.model.id
              var requestData = {fieldValues:fieldValues}
              this.$request.put('/api/admin/member/v2/updateMemberByAdmin',requestData).then((res)=>{
                this.getMemberFields()
            ...
            cancelEditBasic(){
              this.basicFieldList.map(item=>{ this.model[item.code] = this.memberInfo[item.code] })
              this.addStatus = ""
            },
            saveContact(){
              var fieldValues ={}
              this.contactFieldList.map(item=>{ fieldValues[item.code] = this.model[item.code] })
              fieldValues.id = this.model.id
              var requestData = {fieldValues:fieldValues}
              this.$request.put('/api/admin/member/v2/updateMemberByAdmin',requestData).then((res)=>{
                this.getMemberFields()
            ...
            cancelEditContact(){
              this.contactFieldList.map(item=>{ this.model[item.code] = this.memberInfo[item.code] })
              this.addStatus = ""
            },
            saveWorking(){
              var fieldValues ={}
              this.workingField.map(item=>{ fieldValues[item.code] = this.model[item.code] })
              fieldValues.id = this.model.id
              if(fieldValues.industry&&fieldValues.industry.length&&(typeof fieldValues.industry!=="string")){
                fieldValues.industry = fieldValues.industry[fieldValues.industry.length-1]
              }
              var requestData = {fieldValues:fieldValues}
              this.$request.put('/api/admin/member/v2/updateMemberByAdmin',requestData).then((res)=>{
                this.getMemberFields()
            ...
            cancelWorking(){
              this.workingField.map(item=>{ this.model[item.code] = this.memberInfo[item.code] })
              this.addStatus = ""
            },
            saveExp(){
                ...
                if(this.experienceType=='教职工'){
                    if(this.teacherExperienceObj.id){
                        this.$request.put('/api/admin/member/v2/updateMemberTeacherExperience',{memberId:this.userId,teacherExperience:this.teacherExperienceObj}).then((res)=>{
                            this.getMemberFields()
                            this.$message.success('修改就职经历成功');
                            this.addStatus = ""
                        });
                    }else{
                        this.$request.post('/api/admin/member/v2/addMemberTeacherExperience',{memberId:this.userId,teacherExperience:this.teacherExperienceObj}).then((res)=>{
                            this.getMemberFields()
                    ...
                    var obj = Object.assign({education:this.experienceType},this.studentExperienceObj)
                    if(obj.entranceDate){
                    ...
                    if(this.studentExperienceObj.id){
                        this.$request.put('/api/admin/member/v2/updateMemberStudentExperience',{memberId:this.userId,studentExperience:obj}).then((res)=>{
                            this.getMemberFields()
                            this.$message.success('修改就读经历成功');
                            this.addStatus = ""
                        });
                    }else{
                        this.$request.post('/api/admin/member/v2/addMemberStudentExperience',{memberId:this.userId,studentExperience:obj}).then((res)=>{
                            this.getMemberFields()
            ...
            getMemberFields() {
                this.loading = true;
                this.$request.get('/api/admin/member/v2/getUserInfoAndMemberInfo',{params:{memberId: this.userId}}).then((memberInfo)=>{
                    ...
                    if( !(isNaN(this.model.birthday)&&!isNaN(Date.parse(this.model.birthday))) ){
                      ...
                      this.model.birthday  = formatDate(new Date(this.model.birthday),'yyyy-MM-dd');
                    }
          ...
          this.$request.get("/api/admin/associator/getAllFields").then(res => {
            ...
            for(var i=0;i<fieldList.length;i++){
              ...
              if(fieldList[i].topicType==6){
                fieldList[i].showCity = fieldList[i].parameter?JSON.parse(fieldList[i].parameter).showCity:false
              }
              ...
              }else if(fieldList[i].groupId==3&&fieldList[i].code!='education'){
                if(fieldList[i].code=="graduationDate"||fieldList[i].code=="college"||fieldList[i].code=="major"){
