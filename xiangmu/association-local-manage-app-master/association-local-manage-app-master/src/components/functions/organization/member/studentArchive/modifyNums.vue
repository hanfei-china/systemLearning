<template>
  <div>
    <el-form ref="form" :model="form" label-width="80px">
      ...
      <el-form-item label="修改内容" v-if="selectItem.topicType === 2">
        <el-select v-model="form.content" placeholder="请选择修改项">
          <template v-if="selectItem.code=='education'">
            <el-option v-for="item in educationList" :key="item.key" :label="item.title"
                       :value="item.title"></el-option>
          </template>
          <template v-else>
            <el-option v-for="item in selectItem.parameter.options" :key="item.key" :label="item.title"
                       :value="item.title"></el-option>
          </template>
        </el-select>
      </el-form-item>
      ...
      <!--      地区-->
      <el-form-item label="修改内容" v-if="selectItem.topicType === 6">
        <city-choose  v-model="form.content" show-city style="width: auto"/>
      </el-form-item>
