    ...
    <el-main class="unset-padding position-relative">
      <el-container class="absolute-fill-parent background-color-white minwh980" style="padding: 0 16px;"
        ...
        <el-header class="unset-padding flex" height="auto">
          ...
          <div class="flex-fill-in-the-remaining-space flex" style="padding: 16px 0;width: 194px;">
            <div class="flex flexwrap">
              <high-search :educationList="educationList" :collegeList="collegeList"
                           :searchList="searchList"></high-search>
            </div>
          </div>
          <!--          操作-->
          <div class="text-align-right" style="padding: 16px 0 16px 16px;">
            <el-button class="primaryBtnClass" v-if="OPprivileges.includes('201022001')" type="primary" size="small"
                       @click="addMember">新增
            </el-button>
            <el-button class="plainBtnClass" v-if="OPprivileges.includes('201022004')" size="small"
                       @click="batchImportVisible=true">导入
            </el-button>
            <el-button class="plainBtnClass" v-if="OPprivileges.includes('201022005')" size="small"
                       @click="exportFormVisible=true">导出
            </el-button>
            <el-button size="small" class="plainBtnClass" v-if="OPprivileges.includes('201022006')"
                       @click="$router.push('/console/organization/member/studentArchiveMerge')">合并重复
            </el-button>
          </div>
        </el-header>

        <el-main class="unset-padding position-relative">
          <el-table class="custom-el-table header-gray lrd-el-table" height="100%" style="min-width: 400px;" stripe
                    highlight-current-row :data="memberList" size="small" border
                    @selection-change="handleSelectionChange">
            <el-table-column type="selection" width="55" fixed/>
            <template v-for="field in showListFields">
              <el-table-column :key="field.code" show-overflow-tooltip :label="field.name" :prop="field.code"
                               min-width="150">
                <template slot-scope="{ row }">
                  <span v-if="field.code=='name'" class="canClick"
                        @click="editMember(row)">{{ transData(row[field.code], field) }}</span>
                  <span v-else>{{ transData(row[field.code], field) }}</span>
                </template>
              </el-table-column>
            </template>
            <el-table-column label="操作" min-width="100px" fixed="right"
                             v-if="OPprivileges.includes('201022003')||OPprivileges.includes('201022002')">
              <template slot-scope="{ $index, row }">
                <el-button type="text" class="custom-el-button" size="small" v-if="OPprivileges.includes('201022003')"
                           @click="editMember(row)">修改
                </el-button>
                <el-button type="text" class="custom-el-button danger" size="small"
                           v-if="OPprivileges.includes('201022002')" @click="deleteMember(row)">删除
                </el-button>
              </template>
            </el-table-column>
          </el-table>
        ...
        <div class="dialogbox" v-show="memberFormEditVisible">
          <member-form-edit :user-id="memberFormId" v-if="memberFormEditVisible" @close="memberFormEditVisible = false"
        ...
        <member-batch-import @queryTable="refreshMembers" @refresh-members="refreshMembers"
                             :visible.sync="batchImportVisible" title="校友档案导入"></member-batch-import>
        <el-dialog title="导出档案信息" append-to-body :close-on-click-modal="false" :visible.sync="exportFormVisible"
                   width="650px">
...
<script>
import highSearch from "@/components/util/highSearch/highSearch";
...
export default {
  ...
  computed: {
    ...
    showListFields() {
      let fields = this.showFieldListNew;

      for (let i = 0; i < fields.length; i++) {
        if (fields[i].code == 'major') {
          fields.splice(i + 1, 0, {"id": "10000", "name": "现属学院", "code": "currentCollege", "dataType": "0", "seq": -1})
        }
      }
  ...
  methods: {
    ...
    modifyItemSelect(data) {
      ...
      this.$request.put('/api/admin/member/archive/batch', obj).then(res => {
    ...
    exportChosenData() {
      ...
      this.$request.post('/api/admin/member/archive/export', condition, {
        responseType: 'blob',
      }).then(resp => {
        exportExcel(resp);
      });
    ...
    deleteChosenData() {
      ...
      this.$confirm("此操作将删除档案信息, 是否继续?", "提示", {
        ...
        this.$request.delete("/api/admin/member/archive/batch", {params: {ids: memberIds}}).then(res => {
          if (res.success) {
    ...
    deleteMember(row) {
      this.$confirm("此操作将删除档案信息, 是否继续?", "提示", {
        ...
        this.$request.delete("/api/admin/member/archive", {params: {id: row.id}}).then(res => {
          if (res.success) {
    ...
    loadMembers(callback) {
      ...
      this.loading = true;
      // this.$request.get("/api/admin/member/archive/list", { params: this.condition }).then(members => {
      this.$request.post("/api/admin/member/archive/list", this.condition).then(members => {
  ...
  created() {
    ...
    me.$request.get("/api/common/associator/historyCollege/educations").then(r => {
      me.educationList = r.data;
    })
    me.$request.get('/api/admin/member/archive/getAllSearchFields', {params: {type: 0}}).then(res => {
      let arr = []
      ...
      arr.push({type: 3, key: 'currentCollegeId', condition: 1, title: '现属学院', value: '', width: 'w160'})
