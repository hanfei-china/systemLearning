...
<script>
    ...
    export default {
        ...
        computed:{
            condition(){
                let exportBranchId = this.branchId || this.exportBranchId;
                return {
                    exportBranchId,
                    // exportQueryLimit :this.numbers
                }
            },
            branchId(){
        ...
        methods:{
            exportData(){
                ...
                var url='/api/admin/member/poi/export';
                if (this.isBranchMember) {
                    url='/api/admin/branch/export';
                }else if (this.isFamouseMember) {
                    obj.isFamous=true;
                }
                this.$request.post(url,obj,{
                    responseType: 'blob',
                }).then(resp=>{
                    exportExcel(resp);
