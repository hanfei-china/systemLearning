      ...
      <div class="memberMergeList-header__btn">
        <ul>
          <li
            ...
            :key="item.code"
            :class="{'memberMergeList-header__selectBtn':searchType.indexOf(item.code)>=0}"
            @click="filterCondition(item.code)"
          >{{item.name}}一样</li>
        </ul>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    getList() {
      ...
      this.$request
        .get("/api/admin/member/archive/getMergeMemberArchivePage", {
          params
        })
        .then(res => {
          console.log(res);
