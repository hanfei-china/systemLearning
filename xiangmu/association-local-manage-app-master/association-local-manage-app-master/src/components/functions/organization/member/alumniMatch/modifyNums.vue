    ...
    <el-form ref="form" :model="form" label-width="80px">
      ...
      <el-form-item label="修改内容" v-if="selectItem.topicType === 2">
        <el-select v-model="form.content" placeholder="请选择修改项">
          ...
          <template v-else-if="selectItem.code=='college'">
            <el-option v-for="item in collegeList" :key="item.key" :label="item.title" :value="item.title"></el-option>
          </template>
          <template v-else-if="selectItem.code=='major'">
            <el-option v-for="item in majorList" :key="item.key" :label="item.title" :value="item.title"></el-option>
          </template>
          <template v-else>
            <el-option
...
export default {
  name: "modifyNums",
  props: ["selectList","educationList","collegeList","majorList"],
  data() {
    return {
      props: {
        label: "title",
