  ...
  <div class="memberMergeManage">
    <div class="memberMergeManage-header">
      <div class="memberMergeManage-header__left">
        ...
        >检测到{{mergeInfo.listData ? mergeInfo.listData.length : 0}}条重复数据
        </div>
      </div>
      <div>
        <el-button
          :style="{'color':ifAllActivate?'black':'#fff','background-color':ifAllActivate?'#ccc':'#3fa1b6',}"
          v-if="mergeInfo.listData&&mergeInfo.listData.length>0"
          @click="mergeTo"
          :disabled="ifAllActivate"
        >合并本组
        </el-button>
      </div>
    </div>
    <div class="memberMergeManage-select">
      <div style="padding:25px 0;color:rgb(102, 102, 102);">已默认勾选所有学生数据，可手动进行勾选或取消</div>
              ...
              <li>
                <div>
                  ...
                  <span
                    v-if="item.sex==='男'"
                    class="el-icon-male"
                    style="fontSize:16px;color:#3fa1b6;"
                  ></span>
          ...
          <ul class="selectInfo-tabel__item">
            <li class="itemNums" v-for="(item,index) in showMergeInfo" :key="index">
              <ul>
                ...
                <div style="height:50px;backgroundColor: #f2f2f2;borderTop:1px solid #e4e4e4;borderBottom: 1px solid #e4e4e4;margin:15px 0;">
                </div>
                ...
                <div
                  style="height:50px;backgroundColor: #f2f2f2;borderTop:1px solid #e4e4e4;borderBottom: 1px solid #e4e4e4;margin:15px 0;"
                ></div>
                <li
...
export default {
  ...
  methods: {
    ...
    mergeTo() {
      ...
      var params = {
        associationMemberArchive: obj,
        mergeIds
      };
      this.$request
        .post("/api/admin/member/archive/mergeMemberArchiveInfo", params)
        .then(res => {
          if (res.success) {
            this.$message({
              message: "合并成功",
