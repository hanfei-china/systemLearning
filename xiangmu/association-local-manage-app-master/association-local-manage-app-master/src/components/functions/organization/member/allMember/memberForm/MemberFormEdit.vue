      ...
      <vue-scroll @handle-scroll="click">
        <div style="padding: 20px;">
          <div class="fieldbox">
            ...
            <div class="rows">
              <div class="rowpart" v-for="(item,index) in basicFieldList" :key="index">
                ...
                <div>
                  <el-date-picker style="width:100%;" size="mini" v-if="item.topicType==5" v-model="model[item.code]" type="date" placeholder="选择日期" value-format="yyyy-MM-dd"></el-date-picker>
                  ...
                  <el-input v-else size="mini" :placeholder="'请输入'+item.name" v-model="model[item.code]"></el-input>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="fieldbox" v-if="!loading">
            ...
            <div class="dashbox solidbox" v-for="(pitem,pindex) in model.studentExperience" :key="pindex">
              <div class="rows">
                <div class="rowpart">
                  <span>在校经历：</span>
                  <el-select @change="changeExp(pindex)" v-model="pitem.education" placeholder="请选择" size="mini">
                    <el-option v-for="item in educationList" :key="item.title" :label="item.title" :value="item.title"></el-option>
                  </el-select>
                </div>
                <div class="rowpart" v-for="(item,index) in studentExperienceField" :key="index+100">
                  <span>{{item.name}}：</span>
                  <member-edit-field-new :experienceType="pitem.education" :studentExperienceObj="pitem" :memberFields="studentExperienceField" :code="item.code" :config="item" v-model="pitem[item.code]" :isComposeField="true" :itemindex="index" :parentIndex="pindex" @changeMemberFieldsDetail="changeMemberFieldsDetail($event,pitem)" />
                </div>
              </div>
              <div class="btnbox">
            ...
            <div class="dashbox solidbox" v-for="(pitem,pindex) in model.teacherExperience" :key="pindex+100">
              <div class="rows">
                <div class="rowpart">
                  <span>在校经历：</span>
                ...
                <div class="rowpart" v-for="(item,index) in teacherExperienceField" :key="index">
                  <span>{{item.name}}：</span>
                  <member-edit-field-new :experienceType="jiaozhigong" :studentExperienceObj="studentExperienceObj" :memberFields="teacherExperienceField" :code="item.code" :config="item" v-model="pitem[item.code]" :isComposeField="true" :itemindex="index" />
                </div>
              </div>
              <div class="btnbox">
            ...
            <div class="dashbox" v-if="showAddBox">
              <div class="rows">
                ...
                <template v-if="tempExpType&&tempExpType!='教职工'">
                  <div class="rowpart" v-for="(item,index) in studentExperienceField" :key="index+1000">
                    <span>{{item.name}}：</span>
                    <member-edit-field-new :experienceType="tempExpType" :studentExperienceObj="tempExpObj" :memberFields="studentExperienceField" :code="item.code" :config="item" v-model="tempExpObj[item.code]" :isComposeField="true" :itemindex="index" @changeMemberFieldsDetail="changeMemberFieldsDetail($event,tempExpObj)" />
                  </div>
                </template>
                <template v-if="tempExpType&&tempExpType=='教职工'">
                  <div class="rowpart" v-for="(item,index) in teacherExperienceField" :key="index">
                    <span>{{item.name}}：</span>
                    <member-edit-field-new :experienceType="tempExpType" :studentExperienceObj="tempExpObj" :memberFields="teacherExperienceField" :code="item.code" :config="item" v-model="tempExpObj[item.code]" :isComposeField="true" :itemindex="index"/>
                  </div>
                </template>
              </div>
            </div>
          ...
          <div class="fieldbox">
            ...
            <div class="rows">
              <div class="rowpart" v-for="(item,index) in workingField" :key="index">
                <span>{{item.name}}：</span>
                <div>
                  <el-cascader :show-all-levels="false" v-if="item.topicType==20" :placeholder="'下拉选择'+item.name" size="mini" filterable clearable v-model="model[item.code]" :options="industryList" :props="props" style="width: 100%;"/>
                  <el-select style="width:100%;" v-else-if="item.topicType==2" size="mini" v-model="model[item.code]" clearable>
                    <el-option v-for="option of item.parameter.options" :key="option.key" :value="option.title" :label="option.title"/>
                  </el-select>
                  <el-date-picker v-else-if="item.topicType==5" size="mini" v-model="model[item.code]" :value-format="(item.parameter&&JSON.parse(item.parameter).dateType=='year')?'yyyy':'yyyy-MM-dd'" :type="item.parameter?JSON.parse(item.parameter).dateType:'date'" placeholder="选择日期"></el-date-picker>
                  <el-input v-else size="mini" :placeholder="'请输入'+item.name" v-model="model[item.code]"></el-input>
                </div>
              </div>
            </div>
          ...
          <div class="fieldbox">
            ...
            <div class="rows">
              <div class="rowpart" v-for="(item,index) in contactFieldList" :key="index">
                <span>{{item.name}}：</span>
                <div><el-input size="mini" :placeholder="'请输入'+item.name" v-model="model[item.code]"></el-input></div>
              </div>
            </div>
          </div>
        </div>
    ...
    export default {
        ...
        data() {
            return {
              showAddBox:false,
                ...
                cities:cities,
                model:{
                  studentExperience:[],teacherExperience:[]
                },
                basicFieldList:[
        ...
        methods: {
            ...
            saveField(code){
                ...
                var allfield = this.basicFieldList.concat(this.workingField).concat(this.contactFieldList)
                allfield = allfield.map(item=>item.code)
                allfield = allfield.concat(["studentExperience","teacherExperience","id"])
                var obj ={}
                ...
                if(fieldValues.id){
                  ...
                  if (this.isBranchMember) {
                    this.$request.post('/api/admin/branch/addMemberBranch',requestData).then((res)=>{
                        this.$emit('refresh-members')
                        this.$emit('close')
                        this.$message.success('添加成功');
                    });
                  }else{
                    this.$request.post('/api/admin/member/v2/addMemberByAdmin',requestData).then((res)=>{
                        this.$emit('refresh-members')
                        this.$emit('close')
                        this.$message.success('添加成功');
                    });
            ...
            changeExp(index){
              for(var i=0;i<this.studentExperienceField.length;i++){
                if(this.studentExperienceField[i].needClear){
                  this.$set(this.model.studentExperience[index],this.studentExperienceField[i].code,"")
                }
              }
            },
            ...
            getMemberFields() {
              this.loading = true;
              this.$request.get('/api/admin/member/v2/getUserInfoAndMemberInfo',{params:{memberId: this.memberId}}).then((memberInfo)=>{
