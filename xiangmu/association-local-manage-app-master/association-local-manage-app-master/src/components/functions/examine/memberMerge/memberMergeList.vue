      ...
      <div class="memberMergeList-header__btn">
        <ul>
          <li
            ...
            :key="item.id"
            :class="{'memberMergeList-header__selectBtn':searchType.indexOf(item.id)>=0}"
            @click="filterCondition(item.id)"
          >{{item.name}}一样</li>
        </ul>
      </div>
    </div>
      ...
      <div class="memberMergeList-list__list">
        <ul>
          <li
            ...
            :key="index"
            @click="seletList=index"
            :class="{'selectListItem':seletList===index}"
          >
            <div style="marginLeft:15px;">
              <div style="marginBottom:10px;">
                ...
                <span
                  style="color:rgb(204, 153, 0);fontSize:13px;"
                >&nbsp;&nbsp;&nbsp;{{item.detail.length}}条</span>
              </div>
              <div>
                <div style="color:rgb(12, 25, 44);fontWeight:700;">
                  <div
                    style="marginBottom:7px;"
                    v-for="(condition,conIndex) in item.fieldValue"
                    :key="conIndex"
                  >{{condition}}</div>
                </div>
                <!-- <span
...
export default {
  ...
  data() {
    return {
      ...
      searchCondition: [
        { id: "name", name: "姓名" },
        { id: "idNumber", name: "身份证" },
        { id: "education", name: "学历" },
        { id: "entranceDate", name: "入学年" },
        { id: "graduationDate", name: "毕业年" },
        { id: "college", name: "学院" },
        { id: "major", name: "专业" },
        { id: "cless", name: "班级" },
        { id: "studentNo", name: "学号" },
  ...
  methods: {
    ...
    getList() {
      ...
      this.$request
        .get("/api/admin/member/v2/getMergeMemberInfoPage", {
          params
        })
        .then(res => {
          console.log(res);
