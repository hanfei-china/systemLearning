<template>
    <el-container class="absolute-fill-parent background-color-white">
        ...
        <menu-setting-form v-if="formVisible" :visible.sync="formVisible" :menu="menu" @refresh="getMenus"/>
    </el-container>
</template>
        ...
        methods:{
            ...
            deleteMenu(menu){
                this.$msgbox.confirm('删除操作是不可逆的，是否继续删除？','提示',{type: 'warning',confirmButtonText:'确认删除'}).then(()=>{
                    this.$request.delete('/api/admin/menuSetting',{
                        params:{id: menu.id}
                    }).then(()=>{
                        this.getMenus();
            ...
            handleDrop(draggingNode, dropNode, dropType) {
                ...
                this.$request.put('/api/admin/menuSetting/sortMenu',data).then(()=>{
                    this.getMenus(()=>{
            ...
            getMenus(callback){
                this.$request.get('/api/admin/menuSetting/menuList').then(res=>{
