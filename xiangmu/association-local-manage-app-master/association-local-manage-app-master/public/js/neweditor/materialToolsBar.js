...
(function($) {
    ...
    ToolsBar.prototype.operationItems = function() {
        ...
        var _drop = function() {
            ...
            function stop() {
                saveTop=obj.position().top;
                saveLeft=obj.position().left;
        ...
        var _insert_row = function() {
            ...
            that.$toolsBar_wrapper.delegate('[data-act=before-row]', 'click', function(event) {
                if (that.material[0].tagName.toLowerCase() == "img") {
                    ...
                    that.material.parent().before('<p><br/></p>');
                } else {
            ...
            that.$toolsBar_wrapper.delegate('[data-act=after-row]', 'click', function(event) {
                if (that.material[0].tagName.toLowerCase() == "img") {
                    ...
                    that.material.parent().after('<p><br/></p>');
                } else {
                        ...
                        if (_style_val[0] != "color") {
                            ...
                            if (_index_1 >= 0 || _index_2 >= 0 || _index_3 >= 0) {
                                ...
                                if (_index_1 >= 0) {
                                    ...
                                    if(_color_val.length<11&&_color_val.length>6){
                                        _color_val = _color_val.substring(0,3);
                                    };
                                    if(_color_val.length>6){
                                         _color_val = _color_val.substring(0,6);
                                    };
        ...
        var _material_set_up = function() {
            ...
            that.$toolsBar_wrapper.delegate('[data-act=material_set_up]', 'click', function(event) {
                ...
                if (that.material[0].tagName.toLocaleLowerCase() == "img") {
                    _thisMaterial = _thisMaterial.parents(".checkSelected").eq(0);
                }
        ...
        var _image_width = function() {
            var code = '<li class="operation-item" data-tips="设置图片宽度">' +
                ...
                '			<div class="fill-item"><input class="bubble-slider" type="number" min="0" max="740" data-color="#fff" value="100"></div>' +
    ...
    ToolsBar.prototype.randerInitToolsBar = function() {
        ...
        // $(".uediterPrivate").prepend(this.toolsBar_wrapper_code);
    ...
    ToolsBar.prototype.setToolsPosition = function() {
        ...
        if (top < 0) {
            ...
            top = ($(that.ueditor.iframe).offset().top * 1)+450;
        }

        if (left < 0) {
            left = ($(that.ueditor.iframe).offset().left * 1);
        }
        // localStorage.getItem("material_top") ? top = localStorage.getItem("material_top") :top;
        // localStorage.getItem("material_left") ? left = localStorage.getItem("material_left") : left;
        if(saveTop!=""){
            top=saveTop
        }
        if(saveLeft!=""){
    ...
    ToolsBar.prototype.loadSpecialEvent = function() {
        ...
        function imageUpload() {
            ...
            var setting_hard = {
                ...
                url:that.baseUrl+'/api/common/anon/uploadFile', //服务器端的上传页面地址
            ...
            function filesAdded(uploader, files, btn, filesize) {
                ...
                uploader.setOption({
                    ...
                    multipart_params: { "timestamp": getTimestamp, "sign": getRandomNum, type:_type, source:_source},
            ...
            function fileUploaded(uploader, file, responseObject, btn) {
                ...
                if (backData.success) {
                    var upload_image_src = backData.data;
        ...
        function setMaterialColor(colorVal, mark, $colorPicker) {
            for (var i = 0; i < that.material_nodes.length; i++) {
                ...
                var style_code_trim = style_code;
                ...
                for (var j = 0; j < _styles.length; j++) {
                    ...
                    if (_style_item[0].indexOf("border") >= 0) {
                        ...
                        if (mark.split(",").length >= 3) {
                            ...
                            _style_val = _style_val.substring(_index_1, _index_2 + 1).replace(/\s/g, "");
                        } else {
                            var _quality = _style_val.split(" ");
                            if (_quality){
                                for (let q=0;q< _quality.length;q++) {
                                    if (!_quality[q]) continue;
                                    if (_quality[q].indexOf("#") >=0) {
                                        _style_val = _quality[q];
                                        break;
                                    }
                                }
                            }
                            // var _index_1 = _style_val.indexOf("#");
                            // console.log(getcurryStyle(that.material_nodes[i]).borderColor);
    ...
    ToolsBar.prototype.setMenuHeight = function(className) {
        var that = this;
        var winH = $(window).height();
        if (winH - (that.$toolsBar_wrapper.height() + that.$toolsBar_wrapper.offset().top-$(document).scrollTop()) > 350) {
            return false;
        }
        that.$toolsBar_wrapper.find("menu." + className).css({
            height: winH - (that.$toolsBar_wrapper.height() + that.$toolsBar_wrapper.offset().top-$(document).scrollTop()) - 20 + "px",
            "overflow-y": "auto"
        });
    };
    ...
    ToolsBar.prototype.enevtListener = function() {
        ...
        var textDecoration = getcurryStyle(eventSelection).textDecorationLine == "none" ? $(eventSelection).parents("span:first").css("text-decoration") : getcurryStyle(eventSelection).textDecorationLine;
    ...
    ToolsBar.prototype.writeBack = function(attribute) {
        ...
        // ue: ueditor编辑器对象
    $.fn.materialToolsBar = function(ue,baseUrl) {
        var toolsBar = new ToolsBar(baseUrl);
        toolsBar.ueditor = ue;
