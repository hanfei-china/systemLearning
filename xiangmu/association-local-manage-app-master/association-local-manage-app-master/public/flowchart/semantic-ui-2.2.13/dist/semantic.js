        ...
        queryRemote: function (query, callback) {
          var
            apiSettings = {
              ...
              onSuccess: function (response, apiSettings) {
                module.remove.message();
                var re = apiSettings.standardize(response);
                module.setup.menu({
                  values: re[fields.remoteValues]
                });
                callback();
              }
            }
          ...
          if(apiSettings.queryRemoteData && apiSettings.standardize) {
            setTimeout(function () {
              apiSettings.queryRemoteData(query).then((response) => {
                  apiSettings.onSuccess(response, apiSettings);
                }
              ).catch(() => {
                apiSettings.onError();
              })
                ...
                $module
                  .transition({
                    ...
                    onComplete : function() {
                      ...
                      // 由于模态框里面的列表渲染需要时间，所以取得的模态框高度第一遍取得数据不准确，不知道如何处理，暂时执行resize函数
                      module.event.resize()
                      callback();
