...
document.onload = (function (d3, saveAs, Blob, vkbeautify) {
  ...
  var GraphCreator = function (containerId, svg, nodes, edges, participants) {
    ...
    svg.on("mousemove", function (d) {
      thisGraph.show_position.text('pos: ' + d3.mouse(svgG.node())[0].toFixed(0) + ', ' + d3.mouse(
        svgG.node())[1].toFixed(0));
    });
    ...
    $('.timeoutLimit_add .green.button').on('click', function () {
      ...
      if (operate) {//编辑操作
        ...
        $('.timeout_limit tbody').append('<tr jsonstr= ' + JSON.stringify(deadline) + '><td>'
          + deadline.deadlineCondition + '</td></tr>');
      ...
      if (rol_id) {// 编辑
        ...
        thisGraph.participants.forEach(function (participant) {
          if (participant.conventional_definition_id == rol_id) {
            $('.conventional_definition div[data-tab="definition_1"]').find(
              'input[name]:not(".hidden"), textarea').each(function () {
            ...
            if (participant.typeName) {
              participant.typeName.forEach(function (item, i) {
                ...
                $('.conventional_definition .definition_condition tbody').append(
                  ...
                  '  <td name="secLevelS" value="' + participant.secLevelS[i] + '">' + participant.secLevelS[i]
                  + '</td>' +
                  '  <td name="secLevelE" value="' + participant.secLevelE[i] + '">' + participant.secLevelE[i]
                  + '</td>' +
    ...
    $('.conventional_definition').on('click', '.definition_addBtn', function () {
      ...
      $('.conventional_definition .definition_condition tbody').append(
        ...
        '  <td name="itemValue" value="' + name + '">' + (name ? name.split('|')[1] ? name.split('|')[1] : name : '')
        + '</td>' +
    ...
    $('.conventional_definition').on('click', '.green.button', function () {
      var operate = $('.conventional_definition input[name="conventional_def_operate"]').val(),
        ...
        participant = {};
      $('.conventional_definition div[data-tab="definition_1"]').find('input[name]:not(".hidden"), textarea').each(
        function () {
      ...
      var rol = participant.conventional_definition_name ? participant.conventional_definition_name + "-rol"
        : participant.conventional_definition_id + "-rol";
    ...
    $('.monitorinf .monitorinfRemoveBtn').on('click', function () {
      ...
      if (selected.length) {
        ...
        responsible.forEach(function (resp, i) {
          if (resp == definition_id) {
            responsible.splice(i, 1);
          }
        });
        selected.remove();
      ...
      if (operate) { // 编辑
        ...
        participants.forEach(function (p) {
          if (p.conventional_definition_id == rol) {
            $('.monitorinfAddDefinition div[data-tab="definition_one"]').find(
              'input[name]:not(".hidden"), textarea').each(function () {
            ...
            if (p.typeName) {
              p.typeName.forEach(function (tn, j) {
                var itemValue = p.itemValue[j] ? p.itemValue[j].split('|')[1] ? p.itemValue[j].split('|')[1]
                  : p.itemValue[j] : '';
    ...
    $('.monitorinfAddDefinition .green.button').on('click', function () {
      var operate = $('.monitorinfAddDefinition input[name="monitorinf_add_operate"]').val(),
        ...
        participant = {};
      $('.monitorinfAddDefinition div[data-tab="definition_one"]').find('input[name],textarea,select').each(
        function () {
          participant[$(this).attr('name')] = $(this).val();
        });
    ...
    $('.monitorinf_add .green.button').on('click', function () {
      ...
      if (operate) {//编辑
        ...
        $('.monitorinf').find('tbody').append('<tr definition_id="' + definition_id + '"><td>' + definition_rol
          + '</td></tr>');
        $(".monitorinf_grid .content-div").mCustomScrollbar("update");
      ...
      if (show_div.length) {
        ...
        if (show_cls == '.conditionDiv') {//显示-条件-默认选定
          ...
          if (fieldConditions_type) {
            targetActivity$.find('.conditionDiv select[name=fieldConditions_type]').parent().dropdown('set selected',
              fieldConditions_type);
          } else {
            targetActivity$.find('.conditionDiv select[name=fieldConditions_type]').parent().dropdown('set selected',
              'AND');
          }
          targetActivity$.find('.conditionDiv select[name=fieldCondition_type]').parent().dropdown('set selected',
            'AND');
        ...
        if (show_cls == '.workflowbeanDiv') {//显示-按业务对象转移-默认选定
          ...
          targetActivity$.find('.workflowbeanDiv select[name=fieldCondition_type]').parent().dropdown('set selected',
            'AND');
        }
    ...
    $('.targetActivity').on('click', '.workflowbeanDiv .condition_addBtn', function () {
      ...
      var num = workflowbeanDiv$.find('tbody tr').length == 0 ? 1 : parseInt(
        $('.workflowbeanDiv tbody tr:last').find('td:first').text()) + 1;
  ...
  GraphCreator.prototype.showTransition = function (selector, transition) {
    ...
    if (postCond && postCond.conditype == 'WORKFLOWBEAN') {
      if (postCond.condixml) {
        ...
        $(selector).find('.workflowbeanDiv tbody').data('fieldConditions_type',
          beanConditions_obj.fieldConditions_type);
      }
    } else {
    ...
    // 动作筛选
    var str='';
    window.parent.actionList.forEach(function(item){
      if(transition.source.conventional.actionIdList.includes(item.actionId)){
        str+='<option value="'+item.actionId+'">'+item.name+'</option>';
      }
    })
    $('select[name="actionId"]').empty().append(str);

    var assigneeRelationText="";
    $('select[name="assigneeRelation"] option').each(function(index,item){
      if(item.value==transition.postCondition.assigneeRelation){ assigneeRelationText=item.text }
    })
    $('select[name="assigneeRelation"]').dropdown('set text', assigneeRelationText);
    if(transition.postCondition.actionId){
      var actionIdText="";
      $('select[name="actionId"] option').each(function(index,item){
        if(item.value==transition.postCondition.actionId){ actionIdText=item.text }
      })
      $('select[name="actionId"]').dropdown('set text', actionIdText);
      // $('select[name="actionId"]').siblings(".text").text(actionIdText)
    }
    ...
    if (postCond) {
      ...
      $(selector).find('select[name=transitionEventType]').parent().dropdown(
        'set selected', postCond.transitionEventType || '');
  ...
  GraphCreator.prototype.updatePostCondi = function (selector) {
    ...
    if (item_act.length || selector == '.prop_edge') {
      ...
      // $transferInf.find('select[name="assigneeRelation"]').val()
      // console.log($transferInf).find("select")
        ...
        if (tr.length) {
          tr.each(function () {
            ...
            fieldCondition +=
              ...
              '   <expression key="' + json_obj.key + '" sign="' + json_obj.sign_one + '" type="' + json_obj.type
              + '" displayValue="' + json_obj.displayValue_one + '"><![CDATA[' + json_obj.displayValue_two
              + ']]></expression>' +
              '   <expression key="' + json_obj.key + '" sign="' + json_obj.sign_two + '" type="' + json_obj.type
              + '" displayValue="' + json_obj.displayValue_two + '"><![CDATA[' + json_obj.displayValue_two
              + ']]></expression>' +
              ' </fieldCondition>';
          });
          fieldConditions_type = $(selector).find('.conditionDiv select[name=fieldConditions_type]').parent().dropdown(
            'get value');
        }
      ...
      if (conditype == 'EXCEPTION') {//类型选择异常
        postCondition.condiException = $(selector).find('.exceptionDiv select[name=condiException]').parent().dropdown(
          'get value');
      }
      if (conditype == 'WORKFLOWBEAN') { //类型选择业务对象转移
        ...
        if (w_tr.length) {
          w_tr.each(function () {
            var json_obj = JSON.parse($(this).attr('jsonstr'));
            beanCondition +=
              '<beanCondition code="' + json_obj.code + '" type="' + json_obj.beanConditions_type + '" bean="'
              + json_obj.bean + '" paramField="' + json_obj.paramField + '">' +
              '  <expression key="' + json_obj.key + '" sign="' + json_obj.sign_one + '" type="' + json_obj.type
              + '" displayValue="' + json_obj.displayValue_one + '"><![CDATA[' + json_obj.displayValue_one
              + ']]></expression>' +
              '  <expression key="' + json_obj.key + '" sign="' + json_obj.sign_two + '" type="' + json_obj.type
              + '" displayValue="' + json_obj.displayValue_two + '"><![CDATA[' + json_obj.displayValue_two
              + ']]></expression>' +
  ...
  GraphCreator.prototype.getConditionList = function (condition, num) {
    ...
    displayCondi.sign_one = sign == '!=' ? '!=' : sign == '==' ? '=' : sign == 'IN' ? '包含' : sign == 'NOT' ? '不包含'
      : sign == 'PREFIX' ? '前缀' : sign == 'SUFFIX' ? '后缀' : '';
    sign = condition.sign_two;
    displayCondi.sign_two = sign == '!=' ? '!=' : sign == '==' ? '=' : sign == 'IN' ? '包含' : sign == 'NOT' ? '不包含'
      : sign == 'PREFIX' ? '前缀' : sign == 'SUFFIX' ? '后缀' : '';
    displayCondi.fieldCondition_type = condition.fieldCondition_type == 'AND' ? '并且' : condition.fieldCondition_type
    == '||' ? '或者' : '';
    ...
    if (condition.bean) {//类型：按业务对象转移
      ...
      displayCondi.key = condition.key == 'CorpName' ? '获取组织名称' : condition.key == 'Extattr' ? '获取人员扩展属性1'
        : condition.key == 'MObilephone' ? '获取人员移动电话号码' : condition.key == 'Firstname' ? '获取人员姓氏' : condition.key
        == 'Gender' ? '获取人员性别' : condition.key == 'Birthday' ? '获取人员出生日期' : condition.key == 'Securitylevel'
          ? '获取人员安全级别' : condition.key == 'Mainemail' ? '获取人员常用EMAI' : condition.key == 'Lastname' ? '获取人员名字'
            : condition.key == 'Duty' ? '获取人员职务' : condition.key == 'Homeaddress' ? '获取人员家庭住址' : condition.key
            == 'AdminLevel' ? '获取人员级别' : condition.key == 'Workno' ? '获取人员工号' : condition.key == 'Callname' ? '获取人员称呼'
              : condition.key == 'Otheremail' ? '获取人员备用EMAIL' : condition.key == 'Instantmessage' ? '获取人员即时消息号'
                : condition.key == 'Name' ? '获取人员姓名' : condition.key == 'Officeaddress' ? '获取人员工作地点ID' : condition.key
                == 'Fixedphone' ? '获取人员固定电话号码' : '';
  ...
  GraphCreator.prototype.getExtendedAttributes = function (node, deadlineXpdl, conventionalXpdl) {
    ...
    if (highLevel) {
      highLevelXpdl += highLevel.activityEndEvent ? '<ExtendedAttribute Name="ActivityEndEvent" Value="'
        + highLevel.activityEndEvent + '"/>' : '';
      highLevelXpdl += highLevel.activityCreateEvent ? '<ExtendedAttribute Name="ActivityCreateEvent" Value="'
        + highLevel.activityCreateEvent + '"/>' : '';
    ...
    isCreateNew = node.frontCondition.isCreateNew ? '<ExtendedAttribute Name="isCreateNew" Value="'
      + node.frontCondition.isCreateNew + '"/>' : '';
    syncType = node.frontCondition.syncType != "" && node.frontCondition.voteText
      ? '<ExtendedAttribute Name="syncType" Value="' + node.frontCondition.syncType + '|' + node.frontCondition.voteText
      + '"/>' : '';
      ...
      startAndEnd.forEach(function (node) {
        switch (node.type) {
          case 'start':
            ...
            if (outEdge.length) {
              startAndEndXpdl += '<ExtendedAttribute Name="StartOfBlock" Value="none;' + outEdge[0].target.id + ';'
                + node.x + ';' + node.y + ';' + outEdge[0].drawLine + '"/>';
            }
            break;
          case 'end':
            var inToEdge = thisGraph.getLinkedEdges.call(sub_graph, node, -1);
            if (inToEdge.length) {
              startAndEndXpdl += '<ExtendedAttribute Name="EndOfBlock" Value="none;' + inToEdge[0].source.id + ';'
                + node.x + ';' + node.y + ';' + inToEdge[0].drawLine + '"/>';
            }
            break;
        }
      });
  ...
  GraphCreator.prototype.conventionalXpdl = function (node) {
    var thisGraph = this,
      ...
      conventionalXpdl = {};
    conventionalXpdl.startMode = conventional.startMode == 'automatic' ? '<StartMode><Automatic/></StartMode>'
      : '<StartMode><Manual/></StartMode>';
    conventionalXpdl.finishMode = conventional.finishMode == 'automatic' ? '<FinishMode><Automatic/></FinishMode>'
      : '<FinishMode><Manual/></FinishMode>';
    conventionalXpdl.isMulInstance = conventional.isMulInstance
      ? '<ExtendedAttribute Name="isMulInstance" Value="true"/>'
    ...
    conventionalXpdl.isResponsible = '<ExtendedAttribute Name="isResponsible" Value="' + conventional.isResponsible
      + '"/>';
    conventionalXpdl.autoAcceptAllAssignments = '<ExtendedAttribute Name="autoAcceptAllAssignments" Value="'
      + conventional.autoAcceptAllAssignments + '"/>';
    conventionalXpdl.completeAllAssignments = conventional.completeAllAssignments
      ? '<ExtendedAttribute Name="completeAllAssignments" Value="true"/>'
      : '<ExtendedAttribute Name="completeAllAssignments" Value="false"/>';
    conventionalXpdl.assignmentsOrder = conventional.assignmentsOrder
      ? '<ExtendedAttribute Name="assignmentsOrder" Value="true"/>'
      : '<ExtendedAttribute Name="assignmentsOrder" Value="false"/>';
    conventionalXpdl.description = conventional.description ? '<Description>' + conventional.description
      + '</Description>' : '';
    ...
    conventionalXpdl.MustActivity = '<ExtendedAttribute Name="MustActivity" Value="' + conventional.MustActivity
      + '"/>';
    conventionalXpdl.participantID = conventional.participantID ? '<ExtendedAttribute Name="ParticipantID" Value="'
      + conventional.participantID + '"/>' : '<ExtendedAttribute Name="ParticipantID"/>';
  ...
  GraphCreator.prototype.deadlineXpdl = function (node) {
    ...
    deadlineXpdl.warnTime = timeoutLimit.warnTime ? '<ExtendedAttribute Name="warnTime" Value="' + timeoutLimit.warnTime
      + '"/>' : '<ExtendedAttribute Name="warnTime"/>';
    deadlineXpdl.warnAgentClassName = timeoutLimit.warnAgentClassName
      ? '<ExtendedAttribute Name="warnAgentClassName" Value="' + timeoutLimit.warnAgentClassName + '"/>'
      : '<ExtendedAttribute Name="warnAgentClassName"/>';
    deadlineXpdl.limitAgentClassName = timeoutLimit.limitAgentClassName
      ? '<ExtendedAttribute Name="LimitAgentClassName" Value="' + timeoutLimit.limitAgentClassName + '"/>'
    ...
    deadlineXpdl.deadline = deadlines_arr.length > 0 ? '<ExtendedAttribute Name="deadline" Value="'
      + deadlines_arr.join('|') + '"/>' : '<ExtendedAttribute Name="deadline"/>';
    return deadlineXpdl;
  };
  ...
  GraphCreator.prototype.getTransitionRestrictions = function (node, activity_inOut) {
    var join = node.frontCondition.convergeType ? '<Join Type="' + node.frontCondition.convergeType + '"/>'
      : '<Join Type="XOR"/>';
  ...
  GraphCreator.prototype.getParticipants = function () { //??细节还有问题：1.isAppData; 2.condition,conditionXml;
    var thisGraph = this;
    if (thisGraph.participants.length == 0) {
      return '';
    }
    ...
    thisGraph.participants.forEach(function (p) {
      var extendedAttr = '',
        description = p.conventional_definition_description ? '<Description>' + p.conventional_definition_description
          + '</Description>' : '',
  ...
  GraphCreator.prototype.emergeAllXmlContent = function () {
    var thisGraph = this;
    var start = '<WorkflowProcess Id="' + workflow_id + '" Name="' + workflow_name
    ...
    thisGraph.nodes.forEach(function (node) {
      if (node.type == 'activity') {
        activity = '<activity Id="' + node.id + '" Name="' + node.title
      ...
      switch (node.type) {
        case 'start':
          edges.forEach(function (edge) {
            if (edge.source == node) {
              nodes_start += '<ExtendedAttribute Name="StartOfWorkflow" Value="none;' + edge.target.id + ';' + node.x
                + ';' + node.y + ';' + edge.drawLine + '"/>';
            }
          });
          break;
        case 'end':
          edges.forEach(function (edge) {
            if (edge.target == node) {
              nodes_end += '<ExtendedAttribute Name="EndOfWorkflow" Value="none;' + edge.source.id + ';' + node.x + ';'
                + node.y + ';' + edge.drawLine + '"/>';
            }
          });
          break;
  ...
  GraphCreator.prototype.emergeTransitions = function () {
    ...
    edges_act.forEach(function (edge) {
      ...
      var transitionRuleType = p.transitionRuleType ? '<ExtendedAttribute Name="TransitionRuleType" Value="'
        + p.transitionRuleType + '"/>' : '<ExtendedAttribute Name="TransitionRuleType"/>';
      var transitionEventXpdl = p.transitionEvent ? '<ExtendedAttribute Name="TransitionEvent"><![CDATA['
        + p.transitionEvent + ']]></ExtendedAttribute>' : '';
      var conditype = p.conditype || '';
      var conditypeXpdl = conditype ? '<ExtendedAttribute Name="conditype" Value="' + conditype + '"/>'
      ...
      if (conditype == 'WORKFLOWBEAN' || conditype == 'CONDITION') { // 条件、按业务对象转移
        ...
        $(beanConditions).find(selector).each(function () {
          ...
          $(this).find('expression').each(function () {
            ...
            var exp_cdata = $(wfdConfig).find('expression[key="' + $(this).attr('sign') + '"] exp[type="' + $(
              this).attr('type') + '"]').eq(0).html();// <![CDATA[(typeof(${b})!='undefined' && ${b}!=null && ${b}=='${a}')]]>
      ...
      transitions += '<Transition From="' + edge.source.id + '" Id="' + edge.edgeId + '" ' + (p.edgeName ? 'Name="'
        + p.edgeName + '"' : '') + ' To="' + edge.target.id + '">' +
        condition +
        description +
        '   <ExtendedAttributes>' +
        condiException +
  ...
  GraphCreator.prototype.getLink_d = function (start, des) {
    ...
    if (Math.abs(dif_x) > Math.abs(dif_y)) { // 左右连线
      if (dif_x > 0 && dif_y > 0) { //第一象限（200,200-300,300）
        ...
        link = 'M' + d.x + ',' + d.y + 'L' + (mid_x - 5) + ',' + d.y + 'M' + (mid_x - 5) + ',' + d.y + 'A 5,5,0,0,1 '
          + mid_x + ',' + (d.y + 5) +
          'M' + mid_x + ',' + (d.y + 5) + 'L' + mid_x + ',' + (des.y - 5) + 'M' + mid_x + ',' + (des.y - 5)
          + 'A 5,5,0,0,0' + (mid_x + 5) + ',' + des.y +
      ...
      if (dif_x < 0 && dif_y > 0) { //第二象限（200,200-100,300）
        ...
        link = 'M' + d.x + ',' + d.y + 'L' + (mid_x + 5) + ',' + d.y + 'M' + (mid_x + 5) + ',' + d.y + 'A 5,5,0,0,0 '
          + mid_x + ',' + (d.y + 5) +
          'M' + mid_x + ',' + (d.y + 5) + 'L' + mid_x + ',' + (des.y - 5) + 'M' + mid_x + ',' + (des.y - 5)
          + 'A 5,5,0,0,1' + (mid_x - 5) + ',' + des.y +
          'M' + (mid_x - 5) + ',' + des.y + 'L' + des.x + ',' + des.y;
      }
      ...
      if (dif_x > 0 && dif_y > 0) { //第一象限（200,200-300,300）
        ...
        link = 'M' + d.x + ',' + d.y + 'L' + d.x + ',' + (mid_y - 5) + 'M' + d.x + ',' + (mid_y - 5) + 'A 5,5,0,0,0 '
          + (d.x + 5) + ',' + mid_y +
          'M' + (d.x + 5) + ',' + mid_y + 'L' + (des.x - 5) + ',' + mid_y + 'M' + (des.x - 5) + ',' + mid_y
          + 'A 5,5,0,0,1' + des.x + ',' + (mid_y + 5) +
      ...
      if (dif_x < 0 && dif_y > 0) { //第二象限（200,200-100,300）
        ...
        link = 'M' + d.x + ',' + d.y + 'L' + d.x + ',' + (mid_y - 5) + 'M' + d.x + ',' + (mid_y - 5) + 'A 5,5,0,0,1 '
          + (d.x - 5) + ',' + mid_y +
          'M' + (d.x - 5) + ',' + mid_y + 'L' + (des.x + 5) + ',' + mid_y + 'M' + (des.x + 5) + ',' + mid_y
          + 'A 5,5,0,0,0' + des.x + ',' + (mid_y + 5) +
          'M' + des.x + ',' + (mid_y + 5) + 'L' + des.x + ',' + des.y;
      }
      if (dif_x < 0 && dif_y < 0) { //第三象限（200,200-100,100）
  ...
  GraphCreator.prototype.dragmove = function (d) {
    ...
    if (thisGraph.state.shiftNodeDrag || drawLine) {
      ...
      switch (drawLine) {
        case 'NOROUTING': // 直线
          link = dragLine.attr('d', 'M' + d.x + ',' + d.y + 'L' + d3.mouse(svgG.node())[0] + ',' + d3.mouse(
            svgG.node())[1]);
          break;
  ...
  GraphCreator.prototype.circleMouseUp = function (d3node, d) {
    ...
    if (mouseDownNode !== d) {
      ...
      var newEdge = {
        ...
        edgeId: GUID(),
        postCondition: {transitionEventType: 'transitionClass',assigneeRelation:"1",actionId:mouseDownNode.conventional.actionIdList[0]},
  ...
  GraphCreator.prototype.isAllowLinked = function (eventNode, mouseDownNode) {
    ...
    switch (eventNode.type) {
      case 'start':
        ...
        result.msg = '该起始节点已有连线！';
        break;
      case 'end':
        if (thisGraph.hasLinked(eventNode)) {
          result.success = false;
          result.msg = '结束节点不允许流出！';
        }
        break;
    }
  ...
  GraphCreator.prototype.showMenu = function () {
    ...
    if (selectedNode) {
      if (selectedNode.type == 'activity') {
        // 添加只有普通节点才显示属性
        if (selectedNode.component == 'ordinaryActivity') {
          $('#rMenu a[name=propMenu]').show();
        }else if(selectedNode.component == 'blockActivity'){
      ...
      $('#rMenu a[name=editMenu]').hide();
      // 判断只有普通节点的线条才有属性按钮  自定义
      if(selectedEdge.source.component=="ordinaryActivity"){
        $('#rMenu a[name=propMenu]').show();
      }else{
        $('#rMenu a[name=propMenu]').hide();
      }
    ...
    switch (data.type) {
      ...
      case 'end':
        node = {
          ...
          conventional: {
            assignType:"2",
            actionIdList:[window.parent.actionList[0].actionId],
            entityChangeStatus:String(window.parent.entityStatus[0].value),
            ruleType:"2"
            // MustActivity: true,
            // taskAssign: 'taskAutoMode',
            // autoAcceptAllAssignments: true,
  ...
  window.graph_main = new GraphCreator(containerId, svg, initialDate.nodes, initialDate.edges,
...
function initCommonEvent() {
  ...
  $('.editor-toolbar .icon.cloud').on('click', handleUpload);
...
function initFlowChart() {
  ...
  var edges = [];
  var users = {};
  if (window.parent.flowConfigs && window.parent.flowConfigs.designData) {
    var jsonStr = window.parent.flowConfigs.designData;
    var jsonObj = {};
    if (jsonStr) {
      jsonObj = edgeAssociateNode(JSON.parse(jsonStr));
      nodes = jsonObj.nodes || [];
      edges = jsonObj.edges || [];
    }
  } else {
    console.log('No design data get from backend!');
  }
  if (window.parent.flowConfigs.userData) {
    users = window.parent.flowConfigs.userData;
  }
  var initialDate = {
    nodes: nodes,
    edges: edges,
...
function importXpdl(str) {
  ...
  root.find("Activities Activity").each(function () {
    ...
    var d = {
      ...
      component: name == '提交申请' ? 'ordinaryActivity' : name == '块活动' ? 'blockActivity' : name == '审核申请'
        ? 'subFlowActivity' : name == '路径活动' ? 'routeActivity' : '',
      type: 'activity',
      x: x,
      y: y,
