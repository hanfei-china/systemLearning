...
$(function() {
  ...
  // $('.definition_field_user select.dropdown').dropdown(
  //   {
  //     apiSettings: {
  //       queryRemoteData: window.parent.getUsers,
  //       standardize: function (response) {
  ...
  // );
	$('.definition_field_user select.dropdown').dropdown()
	$('.entitybox select.dropdown').dropdown()
	$('.relations select.dropdown').dropdown()
	
  $('.definition_field_role select.dropdown').dropdown(
    {
      apiSettings: {
        queryRemoteData: window.parent.getRoles,
        standardize: function (response) {
          var roleData = response.data.data;
          var result = [];
          for (var roleIdx = 0; roleIdx < roleData.length; roleIdx++) {
            var role = roleData[roleIdx];
            result.push({
              name: role.roleName,
              value: role.roleId
            });
